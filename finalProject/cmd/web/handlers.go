...
import (
	...
	"todobox.alexedwards.net/internal/validator"
)

func (app *application) home(w http.ResponseWriter, r *http.Request) {
	todos, err := app.todos.Latest()
	if err != nil {
...
func (app *application) todoCreatePost(w http.ResponseWriter, r *http.Request) {
	...
	id, err := app.todos.Insert(form.Title, form.Content, form.Expires)
	if err != nil {
		app.serverError(w, err)
		return
	}

	app.sessionManager.Put(r.Context(), "flash", "Form successfully created!")

	http.Redirect(w, r, fmt.Sprintf("/todo/view/%d", id), http.StatusSeeOther)
}
